...
{
        //Init Variables From Config Here
...
if ($oAuthResult->AuthenticateResult->Result == 0) {
                $secondpasskey=$this->secondpasskey=$oAuthResult->AuthenticateResult->Key;
                return true;//In case if you want to check result from connect function via $varstatusconnection=$this->my_voucher->connect(); in view page
                                                        }
        }///end try
...
public function close_voucher_untill_now($key)
{
                 $soap = new SoapClient($this->url);
                 $soap->ClosePendingJobs($key);
}//end function


}//End Of library located applications/libraries/